...
export const ProgrammeCreatePage: React.FC = () => {
  ...
  const createProgrammeMutation = useCreateProgramme();
  ...
  const onSubmit = async (values: CreateProgrammeFormValues) => {
    try {
      const created = await createProgrammeMutation.mutateAsync(values);
      toast.success('Programme créé avec succès !', {
        description: `Le programme "${created.nom}" est prêt.`,
      });
      navigate(`/programmes/${created.id}`);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Erreur lors de la création du programme.';
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Remplissez les informations pour initialiser un nouveau cursus académique.
          </p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div className="space-y-1.5">
            <Label htmlFor="nom" className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
              Nom du Programme <span className="text-[#FF6B0B]">*</span>
            </Label>
            <Input
              id="nom"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="description" className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
              Description <span className="text-[#FF6B0B]">*</span>
            </Label>
            <textarea
              id="description"
              rows={4}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label htmlFor="duree_mois" className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                Durée (en mois) <span className="text-[#FF6B0B]">*</span>
              </Label>
              <Input
                id="duree_mois"
                type="number"
                ...
                max={36}
                {...register('duree_mois')}
                className={`h-11 rounded-xl bg-slate-50 dark:bg-white/5 border ${errors.duree_mois ? 'border-red-500 focus-visible:ring-red-500' : 'border-slate-200 dark:border-white/10'
                  }`}
              />
              {errors.duree_mois && (
                <p className="text-xs font-semibold text-red-500 flex items-center gap-1 mt-1">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="statut" className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
                Statut initial <span className="text-[#FF6B0B]">*</span>
              </Label>
              <select
                id="statut"
                {...register('statut')}
              ...
              >
                <option value="actif">Actif (Ouvert aux inscriptions)</option>
                <option value="inactif">Inactif (Brouillon / En pause)</option>
              </select>
              {errors.statut && (
                <p className="text-xs font-semibold text-red-500 flex items-center gap-1 mt-1">
                  <AlertCircle className="size-3.5" />
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200 dark:border-white/10">
            ...
            <Button
              type="submit"
              disabled={isSubmitting || createProgrammeMutation.isPending}
            ...
            >
              {isSubmitting || createProgrammeMutation.isPending ? (
                <>
