...
import { useProgramme, useCreateSession } from '../hooks/useProgrammes';
  ...
  const onSubmit = async (values: CreateSessionFormValues) => {
    try {
      ...
      toast.success('Session créée avec succès !', {
        description: `La session "${created.nom}" a été rattachée au programme.`,
      });
      navigate(`/sessions/${created.id}`);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Erreur lors de la création de la session.';
    ...
    return (
      <div className="text-center py-16 space-y-4">
        ...
        <p className="text-xs text-slate-400">
          Une session ne peut être créée sans programme parent existant.
        </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="nom" className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">
              Nom de la Session <span className="text-[#FF6B0B]">*</span>
            </Label>
            <Input
              id="nom"
              placeholder="Ex: Promotion Baol Tech - Automne 2026"
              {...register('nom')}
